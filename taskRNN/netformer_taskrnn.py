...
class SingleHeadAttention_cat(nn.Module):
    # the first nneur vars (row) is neural activity, the other vars are stim
    ...
    def forward(self, x):
        ...
        attended_values = torch.matmul(attention_weights, values) + values[:, :self.nq, :]
        if self.use_LN:
...
class TransformerForecasting_cat(nn.Module):
    ...
    def forward(self, inputs):  # inputs shape: (num_samples, sequence_length, input_dim)
        x = self.attention(inputs)[0]  # (num_samples, nneur, 1)
        return x
